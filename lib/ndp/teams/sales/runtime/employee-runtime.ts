...
export interface SalesEmployeeRuntimeAssemblyOptions {
  organizationId?: string;
  contextVersion?: string;
}

export interface SalesEmployeeRuntimeAssembly {
  employeeId: string;
  specialistId: string;
...
export function assembleSalesEmployeeRuntime(
  manifest: DigitalEmployeeManifest,
  options?: SalesEmployeeRuntimeAssemblyOptions,
): SalesEmployeeRuntimeAssembly {
  ...
  const template =
    getPromptTemplate("prompt-template-sales-specialist") ??
...
export function renderEmployeeRuntimePrompt(manifest: DigitalEmployeeManifest): string {
  const assembly = assembleSalesEmployeeRuntime(manifest);
