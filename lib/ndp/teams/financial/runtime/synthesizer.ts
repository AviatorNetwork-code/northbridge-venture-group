...
import { generateFinancialRecommendations } from "../recommendations/engine.js";

/**
 * Synthesizes specialist outputs into a single Financial Team Lead customer-facing response.
 */
export class FinancialTeamSynthesizer implements TeamSynthesizer {
  async synthesize(input: TeamSynthesisInput): Promise<TeamSynthesisResult> {
    ...
    const body =
      insights.length > 0
        ? insights.join(" ")
        : "We reviewed your financial request and are preparing next steps.";
