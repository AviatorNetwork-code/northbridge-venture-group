...
export interface FinancialEmployeeRuntimeAssemblyOptions {
  organizationId?: string;
  contextVersion?: string;
}

export interface FinancialEmployeeRuntimeAssembly {
  employeeId: string;
  specialistId: string;
...
export function assembleFinancialEmployeeRuntime(
  manifest: DigitalEmployeeManifest,
  options?: FinancialEmployeeRuntimeAssemblyOptions,
): FinancialEmployeeRuntimeAssembly {
  ...
  const template =
    getPromptTemplate("prompt-template-financial-specialist") ??
...
export function renderEmployeeRuntimePrompt(manifest: DigitalEmployeeManifest): string {
  const assembly = assembleFinancialEmployeeRuntime(manifest);
