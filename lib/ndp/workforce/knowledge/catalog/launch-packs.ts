import type { KnowledgePack } from "../types/pack.js";
import {
  defaultLayerForCategory,
  KNOWLEDGE_LAYER_ORDER,
} from "../types/category.js";

function pack(input: {
  knowledgePackId: string;
  displayName: string;
  category: KnowledgePack["category"];
  description: string;
  tags: string[];
  dependencies?: string[];
  trustLevel?: KnowledgePack["trustLevel"];
  owner?: string;
  applicableTeams?: string[];
  applicableEmployees?: string[];
  layer?: KnowledgePack["layer"];
  metadata?: Record<string, unknown>;
}): KnowledgePack {
  const layer = input.layer ?? defaultLayerForCategory(input.category);

  return {
    knowledgePackId: input.knowledgePackId,
    displayName: input.displayName,
    category: input.category,
    version: "1.0.0",
    description: input.description,
    tags: input.tags,
    dependencies: input.dependencies ?? [],
    trustLevel: input.trustLevel ?? "canonical",
    owner: input.owner ?? "northbridge-digital",
    layer,
    layerOrder: KNOWLEDGE_LAYER_ORDER[layer],
    applicableTeams: input.applicableTeams ?? [],
    applicableEmployees: input.applicableEmployees ?? [],
    lifecycleStatus: "active",
    launchVisible: true,
    metadata: input.metadata,
  };
}

/**
 * Launch knowledge pack metadata catalog.
 * No knowledge content, prompts, or embeddings.
 */
export const NDP_LAUNCH_KNOWLEDGE_PACKS: KnowledgePack[] = [
  // Universal
  pack({
    knowledgePackId: "knowledge-pack-professional-communication",
    displayName: "Professional Communication",
    category: "universal",
    description: "Foundational professional communication standards for Digital Employees",
    tags: ["communication", "universal"],
  }),
  pack({
    knowledgePackId: "knowledge-pack-customer-service-fundamentals",
    displayName: "Customer Service Fundamentals",
    category: "universal",
    description: "Core customer service principles and response patterns",
    tags: ["customer-service", "universal"],
    dependencies: ["knowledge-pack-professional-communication"],
    applicableTeams: ["team-customer-service", "team-general-service"],
  }),
  pack({
    knowledgePackId: "knowledge-pack-reception-fundamentals",
    displayName: "Reception Fundamentals",
    category: "customer-experience",
    description: "Inbound triage, first contact, and routing fundamentals",
    tags: ["customer-service", "reception"],
    dependencies: ["knowledge-pack-customer-service-fundamentals"],
    applicableTeams: ["team-customer-service"],
    applicableEmployees: ["employee-reception"],
  }),
  pack({
    knowledgePackId: "knowledge-pack-appointment-handling",
    displayName: "Appointment Handling",
    category: "customer-experience",
    description: "Scheduling, rescheduling, and appointment confirmation workflows",
    tags: ["customer-service", "scheduling", "appointment"],
    dependencies: ["knowledge-pack-scheduling-fundamentals"],
    applicableTeams: ["team-customer-service"],
    applicableEmployees: ["employee-appointment"],
  }),
  pack({
    knowledgePackId: "knowledge-pack-reminder-strategy",
    displayName: "Reminder Strategy",
    category: "customer-experience",
    description: "Reminder timing, channel selection, and no-show reduction",
    tags: ["customer-service", "reminder", "scheduling"],
    dependencies: ["knowledge-pack-scheduling-fundamentals"],
    applicableTeams: ["team-customer-service"],
    applicableEmployees: ["employee-reminder"],
  }),
  pack({
    knowledgePackId: "knowledge-pack-customer-success-fundamentals",
    displayName: "Customer Success Fundamentals",
    category: "customer-experience",
    description: "Satisfaction signals, retention, and proactive customer health",
    tags: ["customer-service", "success", "retention"],
    dependencies: ["knowledge-pack-customer-service-fundamentals"],
    applicableTeams: ["team-customer-service"],
    applicableEmployees: ["employee-customer-success"],
  }),
  pack({
    knowledgePackId: "knowledge-pack-complaint-handling",
    displayName: "Complaint Handling",
    category: "customer-experience",
    description: "Complaint intake, acknowledgment, and escalation criteria",
    tags: ["customer-service", "complaint"],
    dependencies: ["knowledge-pack-customer-service-fundamentals"],
    applicableTeams: ["team-customer-service"],
    applicableEmployees: ["employee-customer-service"],
  }),
  pack({
    knowledgePackId: "knowledge-pack-service-recovery",
    displayName: "Service Recovery",
    category: "customer-experience",
    description: "Service recovery steps, goodwill, and follow-up confirmation",
    tags: ["customer-service", "recovery"],
    dependencies: ["knowledge-pack-complaint-handling"],
    applicableTeams: ["team-customer-service"],
    applicableEmployees: ["employee-customer-service"],
  }),
  pack({
    knowledgePackId: "knowledge-pack-business-writing",
    displayName: "Business Writing",
    category: "universal",
    description: "Business writing conventions for customer-facing outputs",
    tags: ["writing", "universal"],
    dependencies: ["knowledge-pack-professional-communication"],
  }),

  // Business
  pack({
    knowledgePackId: "knowledge-pack-business-operations-fundamentals",
    displayName: "Business Operations Fundamentals",
    category: "business",
    description: "Cross-functional business operations context",
    tags: ["operations", "business"],
    dependencies: ["knowledge-pack-business-writing"],
  }),
  pack({
    knowledgePackId: "knowledge-pack-scheduling-fundamentals",
    displayName: "Scheduling Fundamentals",
    category: "scheduling",
    description: "Appointment and calendar coordination fundamentals",
    tags: ["scheduling", "operations"],
    dependencies: ["knowledge-pack-business-operations-fundamentals"],
    applicableEmployees: [
      "employee-appointment",
      "employee-reminder",
    ],
  }),
  pack({
    knowledgePackId: "knowledge-pack-sales-fundamentals",
    displayName: "Sales Fundamentals",
    category: "sales",
    description: "Sales pipeline and opportunity management fundamentals",
    tags: ["sales", "pipeline"],
    dependencies: ["knowledge-pack-business-operations-fundamentals"],
    applicableTeams: ["team-sales"],
  }),
  pack({
    knowledgePackId: "knowledge-pack-lead-qualification",
    displayName: "Lead Qualification",
    category: "sales",
    description: "Lead scoring, qualification criteria, and intent signals",
    tags: ["sales", "leads", "qualification"],
    dependencies: ["knowledge-pack-sales-fundamentals"],
    applicableTeams: ["team-sales"],
    applicableEmployees: ["employee-lead-qualification"],
  }),
  pack({
    knowledgePackId: "knowledge-pack-consultative-selling",
    displayName: "Consultative Selling",
    category: "sales",
    description: "Discovery conversations, needs analysis, and trust-building",
    tags: ["sales", "consultative"],
    dependencies: ["knowledge-pack-sales-fundamentals"],
    applicableTeams: ["team-sales"],
    applicableEmployees: ["employee-sales"],
  }),
  pack({
    knowledgePackId: "knowledge-pack-proposal-quote-strategy",
    displayName: "Proposal & Quote Strategy",
    category: "sales",
    description: "Proposal structure, pricing presentation, and quote workflows",
    tags: ["sales", "proposal", "quote"],
    dependencies: ["knowledge-pack-sales-fundamentals", "knowledge-pack-business-writing"],
    applicableTeams: ["team-sales"],
    applicableEmployees: ["employee-proposal-quote"],
  }),
  pack({
    knowledgePackId: "knowledge-pack-follow-up-strategy",
    displayName: "Follow-up Strategy",
    category: "sales",
    description: "Follow-up cadence, re-engagement, and persistence without pressure",
    tags: ["sales", "follow-up"],
    dependencies: ["knowledge-pack-sales-fundamentals"],
    applicableTeams: ["team-sales"],
    applicableEmployees: ["employee-follow-up"],
  }),
  pack({
    knowledgePackId: "knowledge-pack-crm-hygiene",
    displayName: "CRM Hygiene",
    category: "sales",
    description: "Pipeline record quality, stage definitions, and data consistency",
    tags: ["sales", "crm"],
    dependencies: ["knowledge-pack-sales-fundamentals"],
    applicableTeams: ["team-sales"],
    applicableEmployees: ["employee-crm"],
  }),
  pack({
    knowledgePackId: "knowledge-pack-sales-pipeline-management",
    displayName: "Sales Pipeline Management",
    category: "sales",
    description: "Pipeline stages, velocity, forecasting, and bottleneck resolution",
    tags: ["sales", "pipeline", "management"],
    dependencies: ["knowledge-pack-sales-fundamentals"],
    applicableTeams: ["team-sales"],
  }),
  pack({
    knowledgePackId: "knowledge-pack-marketing-fundamentals",
    displayName: "Marketing Fundamentals",
    category: "marketing",
    description: "Campaign planning and marketing operations fundamentals",
    tags: ["marketing", "campaigns"],
    dependencies: [
      "knowledge-pack-business-operations-fundamentals",
      "knowledge-pack-business-writing",
    ],
    applicableTeams: ["team-marketing"],
  }),
  pack({
    knowledgePackId: "knowledge-pack-campaign-planning",
    displayName: "Campaign Planning",
    category: "marketing",
    description: "Campaign structure, audience targeting, and launch sequencing",
    tags: ["marketing", "campaigns", "planning"],
    dependencies: ["knowledge-pack-marketing-fundamentals"],
    applicableTeams: ["team-marketing"],
    applicableEmployees: ["employee-marketing-campaign"],
  }),
  pack({
    knowledgePackId: "knowledge-pack-content-strategy",
    displayName: "Content Strategy",
    category: "marketing",
    description: "Content calendar planning, platform selection, and scheduling",
    tags: ["marketing", "content", "social"],
    dependencies: ["knowledge-pack-marketing-fundamentals"],
    applicableTeams: ["team-marketing"],
    applicableEmployees: ["employee-content-posts"],
  }),
  pack({
    knowledgePackId: "knowledge-pack-branding",
    displayName: "Branding",
    category: "marketing",
    description: "Brand voice, tone consistency, and identity review standards",
    tags: ["marketing", "brand", "messaging"],
    dependencies: ["knowledge-pack-marketing-fundamentals"],
    applicableTeams: ["team-marketing"],
    applicableEmployees: ["employee-brand"],
  }),
  pack({
    knowledgePackId: "knowledge-pack-marketing-analytics",
    displayName: "Marketing Analytics",
    category: "marketing",
    description: "KPI interpretation, trend detection, and performance analysis",
    tags: ["marketing", "analytics", "kpis"],
    dependencies: ["knowledge-pack-marketing-fundamentals"],
    applicableTeams: ["team-marketing"],
    applicableEmployees: ["employee-marketing-analytics"],
  }),
  pack({
    knowledgePackId: "knowledge-pack-budget-optimization",
    displayName: "Budget Optimization",
    category: "marketing",
    description: "Budget allocation, ROI analysis, and spend prioritization",
    tags: ["marketing", "finance", "budget"],
    dependencies: [
      "knowledge-pack-marketing-fundamentals",
      "knowledge-pack-financial-fundamentals",
    ],
    applicableTeams: ["team-marketing"],
    applicableEmployees: ["employee-advertising-budget"],
  }),
  pack({
    knowledgePackId: "knowledge-pack-financial-fundamentals",
    displayName: "Financial Fundamentals",
    category: "finance",
    description: "Billing, receivables, and financial reporting fundamentals",
    tags: ["finance", "billing"],
    dependencies: ["knowledge-pack-business-operations-fundamentals"],
    applicableTeams: ["team-financial"],
  }),
  pack({
    knowledgePackId: "knowledge-pack-billing-fundamentals",
    displayName: "Billing Fundamentals",
    category: "finance",
    description: "Invoicing accuracy, billing cycles, and payment terms",
    tags: ["finance", "billing"],
    dependencies: ["knowledge-pack-financial-fundamentals"],
    applicableTeams: ["team-financial"],
    applicableEmployees: ["employee-billing"],
  }),
  pack({
    knowledgePackId: "knowledge-pack-accounts-receivable-fundamentals",
    displayName: "Accounts Receivable Fundamentals",
    category: "finance",
    description: "AR aging, collection principles, and overdue account management",
    tags: ["finance", "receivables"],
    dependencies: ["knowledge-pack-financial-fundamentals"],
    applicableTeams: ["team-financial"],
    applicableEmployees: ["employee-accounts-receivable"],
  }),
  pack({
    knowledgePackId: "knowledge-pack-cash-flow-basics",
    displayName: "Cash Flow Basics",
    category: "finance",
    description: "Cash timing, liquidity signals, and early warning indicators",
    tags: ["finance", "cash-flow"],
    dependencies: ["knowledge-pack-financial-fundamentals"],
    applicableTeams: ["team-financial"],
    applicableEmployees: ["employee-financial"],
  }),
  pack({
    knowledgePackId: "knowledge-pack-financial-reporting",
    displayName: "Financial Reporting",
    category: "finance",
    description: "P&L, balance sheet, and cash flow reporting standards",
    tags: ["finance", "reporting"],
    dependencies: ["knowledge-pack-financial-fundamentals"],
    applicableTeams: ["team-financial"],
    applicableEmployees: ["employee-financial-reporting"],
  }),
  pack({
    knowledgePackId: "knowledge-pack-payment-followup-strategy",
    displayName: "Payment Follow-up Strategy",
    category: "finance",
    description: "Overdue payment follow-up cadence and professional escalation",
    tags: ["finance", "collections", "follow-up"],
    dependencies: ["knowledge-pack-accounts-receivable-fundamentals"],
    applicableTeams: ["team-financial"],
    applicableEmployees: ["employee-accounts-receivable"],
  }),
  pack({
    knowledgePackId: "knowledge-pack-expense-awareness",
    displayName: "Expense Awareness",
    category: "finance",
    description: "Expense tracking, categorization, and periodic review",
    tags: ["finance", "expenses"],
    dependencies: ["knowledge-pack-financial-fundamentals"],
    applicableTeams: ["team-financial"],
    applicableEmployees: ["employee-financial"],
  }),

  // Industry
  pack({
    knowledgePackId: "knowledge-pack-aviation-fundamentals",
    displayName: "Aviation Fundamentals",
    category: "aviation",
    description: "Flight training and aviation operations context",
    tags: ["aviation", "industry"],
    dependencies: [
      "knowledge-pack-scheduling-fundamentals",
      "knowledge-pack-business-operations-fundamentals",
    ],
    applicableTeams: ["team-flight-school"],
  }),
  pack({
    knowledgePackId: "knowledge-pack-dental-fundamentals",
    displayName: "Dental Fundamentals",
    category: "dental",
    description: "Dental practice operations and patient coordination context",
    tags: ["dental", "industry"],
    dependencies: [
      "knowledge-pack-scheduling-fundamentals",
      "knowledge-pack-customer-service-fundamentals",
    ],
    applicableTeams: ["team-dental-office"],
  }),
  pack({
    knowledgePackId: "knowledge-pack-legal-fundamentals",
    displayName: "Legal Fundamentals",
    category: "legal",
    description: "Law firm operations and client coordination context",
    tags: ["legal", "industry"],
    dependencies: [
      "knowledge-pack-business-operations-fundamentals",
      "knowledge-pack-business-writing",
    ],
    applicableTeams: ["team-law-firm"],
  }),
  pack({
    knowledgePackId: "knowledge-pack-hvac-fundamentals",
    displayName: "HVAC Fundamentals",
    category: "hvac",
    description: "HVAC service business dispatch and maintenance context",
    tags: ["hvac", "industry"],
    dependencies: [
      "knowledge-pack-scheduling-fundamentals",
      "knowledge-pack-sales-fundamentals",
    ],
    applicableTeams: ["team-hvac"],
  }),

  // Organization
  pack({
    knowledgePackId: "knowledge-pack-northbridge-customer-success-doctrine",
    displayName: "Northbridge Customer Success Doctrine",
    category: "organization",
    description: "Northbridge Digital customer success operating doctrine",
    tags: ["northbridge", "customer-success", "organization"],
    dependencies: ["knowledge-pack-customer-service-fundamentals"],
    applicableEmployees: [
      "employee-customer-service",
      "employee-customer-success",
      "employee-reception",
    ],
  }),
  pack({
    knowledgePackId: "knowledge-pack-northbridge-communication-standards",
    displayName: "Northbridge Communication Standards",
    category: "organization",
    description: "Northbridge Digital communication standards for customer-facing work",
    tags: ["northbridge", "communication", "organization"],
    dependencies: ["knowledge-pack-professional-communication"],
  }),
];

export const NDP_LAUNCH_KNOWLEDGE_PACK_ID_SET = new Set(
  NDP_LAUNCH_KNOWLEDGE_PACKS.map((entry) => entry.knowledgePackId),
);

export function getKnowledgePack(
  knowledgePackId: string,
): KnowledgePack | undefined {
  return NDP_LAUNCH_KNOWLEDGE_PACKS.find(
    (entry) => entry.knowledgePackId === knowledgePackId,
  );
}

export function listLaunchVisibleKnowledgePacks(): KnowledgePack[] {
  return NDP_LAUNCH_KNOWLEDGE_PACKS.filter((entry) => entry.launchVisible);
}
