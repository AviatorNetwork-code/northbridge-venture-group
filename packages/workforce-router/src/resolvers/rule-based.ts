...
function countTagOverlap(requestTags: string[] | undefined, ruleTags: string[] | undefined): number {
  if (!requestTags?.length || !ruleTags?.length) return 0;
  const ruleSet = new Set(ruleTags);
  return requestTags.filter((tag) => ruleSet.has(tag)).length;
}

function ruleMatchesRequest(rule: RouteRule, request: RoutingRequest): boolean {
  ...
  const capabilityOverlap = countTagOverlap(
  ...
  if (requiresIntent && intentOverlap === 0) return false;
  if (requiresCapability && capabilityOverlap === 0) return false;
  if (!requiresIntent && !requiresCapability && !rule.match.channel) return false;

  return true;
}

function scoreRuleMatch(rule: RouteRule, request: RoutingRequest): number {
  const intentOverlap = countTagOverlap(request.payload.intentTags, rule.match.intentTags);
  const capabilityOverlap = countTagOverlap(
    request.payload.capabilityTags,
    rule.match.capabilityTags,
...
export class RuleBasedRouteResolver implements RouteResolver {
  ...
  async resolve(
    ...
    for (const rule of rules.rules) {
      ...
      if (rule.routeTo.ownerType === "nordi") continue;
