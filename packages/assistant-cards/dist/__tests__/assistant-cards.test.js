...
import { assertAssistantRichCard, confirmationRequestCardFixture, confirmationWithoutActionsFixture, createAssistantCardValidationError, createSafeInvalidCardFallback, errorCardFixture, errorWithoutSeverityFixture, explanationCardFixture, missingTitleCardFixture, recommendationCardFixture, toolResultCardFixture, unknownTypeCardFixture, validateAssistantRichCard, validateAssistantRichCards, validateNormalizedPlannerCards, warningCardFixture, warningWithoutSeverityFixture, } from "../index.js";
import { createExampleContext, createInMemoryToolExecutor, createSimpleResponsePlanner, EXAMPLE_READ_TOOL, } from "@northbridge/assistant-router";
...
describe("assertAssistantRichCard and safe errors", () => {
    it("throws AssistantCardValidationError on invalid card", () => {
        expect(() => assertAssistantRichCard(missingTitleCardFixture)).toThrowError("AssistantRichCard validation failed.");
    });
    it("creates validation error with issues", () => {
        const result = validateAssistantRichCard(unknownTypeCardFixture);
        const error = createAssistantCardValidationError("Invalid card", result.issues);
...
describe("router planner output validation", () => {
    it("validates normalized @northbridge/assistant-router planner cards", async () => {
        ...
        const toolResult = await executor.execute(EXAMPLE_READ_TOOL.tool_id, {}, createExampleContext());
...
//# sourceMappingURL=assistant-cards.test.js.map