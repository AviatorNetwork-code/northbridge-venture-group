import { ACTIONS_REQUIRED_CARD_TYPES, ASSISTANT_CARD_SEVERITIES, ASSISTANT_CARD_TYPES, ASSISTANT_RICH_CARD_SCHEMA_VERSION, SEVERITY_REQUIRED_CARD_TYPES, } from "./types.js";
...
function validateAction(value, index) {
    ...
    if (typeof value.action_id !== "string" || value.action_id.length === 0) {
        issues.push(issue(`${path}.action_id`, "REQUIRED", "action_id is required."));
    }
    if (typeof value.label !== "string" || value.label.length === 0) {
        issues.push(issue(`${path}.label`, "REQUIRED", "label is required."));
    }
    if (value.draft_id !== undefined &&
        typeof value.draft_id !== "string") {
        issues.push(issue(`${path}.draft_id`, "INVALID_TYPE", "draft_id must be a string."));
    }
    if (value.disabled !== undefined && typeof value.disabled !== "boolean") {
        issues.push(issue(`${path}.disabled`, "INVALID_TYPE", "disabled must be a boolean."));
    }
    if (value.disabled_reason !== undefined &&
        typeof value.disabled_reason !== "string") {
        issues.push(issue(`${path}.disabled_reason`, "INVALID_TYPE", "disabled_reason must be a string."));
    }
    ...
    value.forEach((entry, index) => {
        ...
        if (actionIssues.length === 0 && isRecord(entry)) {
            actions.push({
                ...
                label: entry.label,
                draft_id: typeof entry.draft_id === "string" ? entry.draft_id : undefined,
                disabled: typeof entry.disabled === "boolean" ? entry.disabled : undefined,
                disabled_reason: typeof entry.disabled_reason === "string"
...
function parseMetadata(value) {
    ...
    for (const [key, entry] of Object.entries(value)) {
        ...
        if (entry !== null &&
            ...
            t !== "number" &&
            t !== "boolean") {
            issues.push(issue(`metadata.${key}`, "INVALID_TYPE", "metadata values must be string, number, boolean, or null."));
            continue;
        }
        metadata[key] = entry;
    }
...
export function validateAssistantRichCard(input) {
    const issues = [];
    if (!isRecord(input)) {
        return {
            valid: false,
    ...
    for (const key of Object.keys(input)) {
        if (!allowedTopLevel.has(key)) {
            issues.push(issue(key, "UNKNOWN_FIELD", `Unknown top-level field: ${key}.`));
        }
    }
    if (input.schema_version !== ASSISTANT_RICH_CARD_SCHEMA_VERSION) {
        issues.push(issue("schema_version", "INVALID_VERSION", `schema_version must be "${ASSISTANT_RICH_CARD_SCHEMA_VERSION}".`));
    }
    ...
    else if (!ASSISTANT_CARD_TYPES.includes(type)) {
        issues.push(issue("type", "UNKNOWN_TYPE", `Unknown card type: ${type}.`));
    }
    ...
    if (input.severity !== undefined) {
        if (typeof input.severity !== "string" ||
            !ASSISTANT_CARD_SEVERITIES.includes(input.severity)) {
            issues.push(issue("severity", "INVALID_SEVERITY", "severity is not recognized."));
        }
        else {
    ...
    if (typeof type === "string" &&
        SEVERITY_REQUIRED_CARD_TYPES.includes(type) &&
        severity === undefined) {
        issues.push(issue("severity", "REQUIRED", `${type} cards must include severity.`));
    }
    const { issues: actionIssues, actions } = parseActions(input.actions);
    issues.push(...actionIssues);
    if (typeof type === "string" &&
        ACTIONS_REQUIRED_CARD_TYPES.includes(type) &&
        (!actions || actions.length === 0)) {
        issues.push(issue("actions", "REQUIRED", "confirmation_request cards must include at least one action."));
    }
    ...
    if (input.created_at !== undefined && typeof input.created_at !== "string") {
        issues.push(issue("created_at", "INVALID_TYPE", "created_at must be an ISO string."));
    }
    if (issues.length > 0) {
...
export function validateAssistantRichCards(cards) {
...
export function assertAssistantRichCard(input) {
    const result = validateAssistantRichCard(input);
    if (!result.valid || !result.card) {
        throw createAssistantCardValidationError("AssistantRichCard validation failed.", result.issues);
    }
    return result.card;
}
