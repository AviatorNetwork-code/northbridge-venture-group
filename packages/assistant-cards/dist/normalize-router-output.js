import { validateAssistantRichCards, validateAssistantRichCard, } from "./validate.js";
/**
 ...
 */
export function normalizeContractPlannerCard(card, index) {
    const type = mapContractStatusToCardType(card);
    const body = card.subtitle ??
...
function mapContractStatusToCardType(card) {
    if (card.actions?.some((action) => action.draft_id)) {
